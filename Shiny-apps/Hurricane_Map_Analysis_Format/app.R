...
# ------------------------------------------------------------------------------
# Description:
# This Shiny app visualizes hurricane tracks using historical storm data. 
# Users can filter hurricanes based on category, wind speed, year range, 
# and month. The app provides an interactive map where storms are color-coded 
# by wind speed, with a customizable color palette and the option to reverse colors.
# ------------------------------------------------------------------------------
# Details:
...
# - Uses a **sidebar layout with a Bootstrap theme** for an enhanced UI.
# ------------------------------------------------------------------------------
# Inputs:
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    selectInput("selected_category", "Select Hurricane Category:",
                choices = c("All", "Tropical Storm", "Category 1", "Category 2", "Category 3", "Category 4", "Category 5"),
                selected = "All"),
    sliderInput("wind_range", "Select Wind Speed Range (knots):",
                min = min(c(0,dat$WMO_WIND), na.rm = TRUE),
...
server <- function(input, output) {
  ...
  output$map_plot <- renderPlot({
    ...
    ggplot() +
      geom_sf(data = world_map, fill = "grey70", color = "grey50") +
      geom_path(data = hurricane_data, aes(x = LON, y = LAT, group = SID),
                color = "grey20", linewidth = 0.5, alpha = 0.5) +
